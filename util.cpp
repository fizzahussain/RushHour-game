...
// Draw a checker-style road/building background.
//   - boardSize: the length (in pixels) of your square play area
//   - cell: size of each tile (e.g. 40px)
void DrawBackground(int boardSize, int cell) {
    bool drawRoad = true;
    for(int y = 0; y < boardSize; y += cell) {
        drawRoad = !drawRoad;
        for(int x = 0; x < boardSize; x += cell) {
            // white roads, black buildings
            glColor3fv(drawRoad ? colors[WHITE] : colors[BLACK]);
            DrawRectangle(x, y, cell, cell, colors[drawRoad ? WHITE : BLACK]);
            drawRoad = !drawRoad;
        }
    }
}
