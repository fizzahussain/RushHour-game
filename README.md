...
| `Space` / `S` | Pick up or drop off a passenger/package |
| `R` | Refuel when positioned at a fuel station |
| `P` | Request a role switch while at the board origin |
| `Enter` | Confirm menu selections, name entry, refueling, or role switch |
| `W` / `S` | Navigate the theme and main menus |
| `B` | Return from the leaderboard |
...
| Event | Taxi Driver | Delivery Driver |
|---|---:|---:|
| Successful job | `+10` | `+20` |
| Hit an obstacle | `-2` | `-4` |
| Hit another car | `-3` | `-5` |
...
|---|---:|
| Move validation | `O(O + C)` |
| DFS reachability check | `O(R × C)` |
| Maintain passenger/package availability | `O(P + K + F + O)` per maintenance pass |
| Insert a leaderboard score | `O(H)` |
...
|---|---|
| **C++** | Core game logic, OOP model, state management, and file handling |
| **OpenGL / FreeGLUT** | 2D rendering, game window, keyboard input, and timer callbacks |
| **SDL2 + SDL2_mixer** | Background music and gameplay sound effects |
| **FreeImage / CImg** | Graphics and image utility support |
...
> [!NOTE]
> The repository also includes its original `install-libraries.sh` helper. The command above lists the libraries referenced by the current Makefile, including SDL2 and SDL2_mixer.
...
For a deeper walkthrough of the class hierarchy, board generation, DFS reachability, traffic updates, role switching, fuel system, and leaderboard persistence, see [`docs/TECHNICAL.md`](docs/TECHNICAL.md).
